...
{
    public class SecondLevelRocketStrategy : IRocketsStrategy
    {
        public List<AttackResult> Attack(GridItem[] opponentGridItems, IPlayer opponent, int damage, int rocketGridId)
        {
            ...
            List<int> affectedGridItems = Enumerable.Range(0, 8).Select(x => x + (affectedRow + 1 * 8)).Reverse().ToList();
            affectedGridItems.AddRange(Enumerable.Range(0, 8).Select(x => x + (affectedRow - 1 * 8)).Reverse().ToList());
